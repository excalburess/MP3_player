...
if(disp_size == DISP_LARGE) {
    ...
    lv_obj_set_style_pad_left(tab_bar, hor_res / 2, 0);

    // Title label (aligned directly to the tab bar where the logo used to sit)
    lv_obj_t * title_label = lv_label_create(tab_bar);
    lv_obj_add_style(title_label, &style_title, 0);
    lv_label_set_text(title_label, "LVGL v9");
    lv_obj_align(title_label, LV_ALIGN_LEFT_MID, -hor_res / 2 + 25, -10);

    // Subtitle label (aligned underneath the title label)
    lv_obj_t * sub_label = lv_label_create(tab_bar);
    lv_label_set_text(sub_label, "Widgets demo");
    lv_obj_add_style(sub_label, &style_text_muted, 0);
    lv_obj_align_to(sub_label, title_label, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 2);
  }
...
void Backlight_adjustment_event_cb(lv_event_t * e) {
  uint8_t Backlight = lv_slider_get_value((const lv_obj_t *)lv_event_get_target(e)); 
